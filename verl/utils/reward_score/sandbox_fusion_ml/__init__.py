...
def compute_score_multilingual(
    sandbox_fusion_url, concurrent_semaphore, memory_limit_mb, completion, language, test_cases, continuous=False, timeout=10
):
    """
    Computes the code score using the remote sandbox API.
    ...
    Args:
        ...
        completion: The completion string containing the code.
        language: The programming language of the completion.
        test_cases: JSON string or dictionary containing "inputs" and "outputs".
        continuous: Whether to compute a continuous score (based on the first N test cases).
        timeout: Timeout for each test case.
    ...
    if "```" + language in completion:
        solution = completion.split("```" + language)[-1].split("```")[0]
    elif "```" in completion:
        # Handle cases like ```\ncode\n```
        parts = completion.split("```")
        if len(parts) >= 2:
        ...
        return 0.0, 0.0, [{"error": "Invalid completion (missing code block)"}]


    if language == 'csharp':
        solution = '#pragma warning disable\n'+solution
    ...
    try:
        if not isinstance(test_cases, dict):
            try:
                ...
                logger.error(f"Failed to parse test_cases JSON: {e}")
                return 0.0, 0.0, [{"error": "Invalid test_cases JSON format"}]

        if not test_cases or "inputs" not in test_cases or "outputs" not in test_cases:
            logger.error("Invalid test_cases structure.")
            return 0.0, 0.0, [{"error": "Invalid test_cases structure (missing inputs/outputs)"}]
        ...
        if continuous:
            ...
            pass_score = 1.0 if score >= 1.0 else 0.0
        ...
        pass_score = 0.0
    # Ensure float and list are returned
    return float(score), float(pass_score), final_metadata if isinstance(final_metadata, list) else [final_metadata]
